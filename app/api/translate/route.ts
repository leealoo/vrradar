  ...
  try {
    ...
    if (!articleId) {
      const articles = await prisma.article.findMany({
        where: { translatedTitle: "", deleted: false },
      ...
      const remaining = await prisma.article.count({ where: { translatedTitle: "", deleted: false } });
      return NextResponse.json({ translated, total: articles.length, failed, remaining });
    }

    const article = await prisma.article.findUnique({ where: { id: articleId } });
    if (!article || article.deleted) return NextResponse.json({ error: "文章不存在" }, { status: 404 });
